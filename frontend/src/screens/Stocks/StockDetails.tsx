...
const StockDetails = () => {
  ...
  const { _companyIndex, type, _index } = useParams();
  const companyIndex = _companyIndex ? parseInt(_companyIndex) : undefined;
  ...
  useEffect(() => {
    if (companyIndex && type && index && context) {
      ...
      const company = context[companyIndex];
      if (type == "c") setMarketData(company.options[index].call);
      if (type == "p") setMarketData(company.options[index].put);
      if (type == "f") setMarketData(company.futures[index].market_data);
  ...
  }, [companyIndex, type, index, context]);
