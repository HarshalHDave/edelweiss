...
function call_delta(S, K, T, r, sigma) {
  return norm_cdf(d1(S, K, T, r, sigma));
}

function call_gamma(S, K, T, r, sigma) {
  return norm_pdf(d1(S, K, T, r, sigma)) / (S * sigma * Math.sqrt(T));
}

function call_vega(S, K, T, r, sigma) {
  return 0.01 * (S * norm_pdf(d1(S, K, T, r, sigma)) * Math.sqrt(T));
}

function call_theta(S, K, T, r, sigma) {
  return (
    0.01 *
    (-(S * norm_pdf(d1(S, K, T, r, sigma)) * sigma) /
      (2 * Math.sqrt(T)) -
      r * K * Math.exp(-r * T) * norm_cdf(d2(S, K, T, r, sigma)))
  );
}

function call_rho(S, K, T, r, sigma) {
  return (
    0.01 *
    (K * T * Math.exp(-r * T) * norm_cdf(d2(S, K, T, r, sigma)))
  );
}

function put_delta(S, K, T, r, sigma) {
  return -norm_cdf(-d1(S, K, T, r, sigma));
}

function put_gamma(S, K, T, r, sigma) {
  return norm_pdf(d1(S, K, T, r, sigma)) / (S * sigma * Math.sqrt(T));
}

function put_vega(S, K, T, r, sigma) {
  return 0.01 * (S * norm_pdf(d1(S, K, T, r, sigma)) * Math.sqrt(T));
}

function put_theta(S, K, T, r, sigma) {
  return (
    0.01 *
    (-(S * norm_pdf(d1(S, K, T, r, sigma)) * sigma) /
      (2 * Math.sqrt(T)) +
...
function put_rho(S, K, T, r, sigma) {
  return (
    0.01 *
    (-K * T * Math.exp(-r * T) * norm_cdf(-d2(S, K, T, r, sigma)))
  );
...
function implied_volatility(Price, S, K, T, r, put_or_call) {
  let sigma = 0.001;
...
console.log(calculation(81.55, 19245, 19300, "07-06-2023", "C"));
